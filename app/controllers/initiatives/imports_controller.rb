...
module Initiatives
  ...
  class ImportsController < ApplicationController
    before_action :require_workspace_selected, only: %i[new create edit update] # rubocop:disable Rails/LexicallyScopedActionFilter
    before_action :set_initiatives_import, only: %i[show edit update destroy] # rubocop:disable Rails/LexicallyScopedActionFilter

    def new
      @initiatives_import = current_workspace.initiatives_imports.build(
    ...
    def create # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
      ...
      if @initiatives_import.save && @initiatives_import.process(current_workspace)
    ...
    def scorecard_type_from_scope(scope)
      ...
      when 'sdgs_alignment_cards' then 'SustainableDevelopmentGoalAlignmentCard'
        ...
        current_workspace.default_scorecard_type&.name || 'TransitionCard'
      end
    end
  end
end
