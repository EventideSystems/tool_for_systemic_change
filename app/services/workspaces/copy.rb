  ...
  class Copy # rubocop:disable Style/Documentation
    ...
    def call
      workspace.dup.tap do |new_workspace|
        ...
        new_workspace.impact_card_data_models.delete_all

        copy_stakeholder_types(new_workspace)
        copy_impact_card_data_models(new_workspace)
      end
    end
    ...
    def copy_impact_card_data_models(new_workspace) # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
      ImpactCardDataModel.where(workspace:).find_each do |data_model|
    ...
    def create_focus_area_group(focus_area_group, new_data_model)
      FocusAreaGroup.create!(
        focus_area_group
          ...
          .merge(impact_card_data_model_id: new_data_model.id)
      )
    end
