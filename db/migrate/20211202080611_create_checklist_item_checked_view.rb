class CreateChecklistItemCheckedView < ActiveRecord::Migration[6.1]
  def up
    connection.execute(<<~SQL)
      CREATE OR REPLACE VIEW checklist_item_checked_view AS
        select
          distinct on (versions.id) 
          'updated_checked' as event,
          checklist_items.id as checklist_item_id,
          '' as comment,
          coalesce((next_versions.object->>'updated_at')::timestamp, checklist_items.updated_at) as occuring_at,
  ...
  def down
    connection.execute(<<~SQL)
      DROP VIEW checklist_item_checked_view
    SQL
  end
end
