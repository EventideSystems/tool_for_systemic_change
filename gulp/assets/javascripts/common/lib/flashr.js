// overriding flashr module because it assumes we're using ngRoute, which we're not. Theres also a bug with later toasts (var clash);
...
angular.module('flashr', [])
    .factory('flashr', ['$rootScope',
        function ($rootScope) {
            ...
            // when the location changes, flash the "later" messages
            $rootScope.$on('$locationChangeSuccess', function () {
                for (var i = 0; i < _toasts.length; i++) {
                    var item = _toasts[i];
                    toast(item.type, item.msg);
                    _toasts = _.difference(item, _toasts);
                }
            });
